...
export default function ResultsBold({ pricingPlan = 'bold' }) {
  ...
  useEffect(() => {
    trackFunnelEvent('results_bold_viewed', null, plan_key)
    Promise.all([initFacebookPixel(), initTikTokPixel()]).then(() => {
      trackFbEvent('ViewContent', { content_name: 'results_bold', content_category: plan_key })
      trackTtEvent('ViewContent', { content_name: 'results_bold', content_id: plan_key, content_type: 'product' })
    })
  }, [plan_key])
  ...
  const handleProtocoloClick = () => {
    trackFunnelEvent('results_bold_protocolo_clicked', null, plan_key)
    setShowLoading(true)
  }

  const onLoadingDone = () => {
    navigate('/offer-bold', { state: { answers, score } })
  }
      ...
      <div className="max-w-xl mx-auto px-5 pt-5 pb-10 flex flex-col gap-6">
        ...
        <FadeIn>
          ...
          >
            ...
            {t('boldFlow.diagnosis.urgencyBanner')}
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.1}>
          <h1 className="text-3xl font-extrabold text-stone-900 leading-tight text-center">
            {name
              ? t('boldFlow.diagnosis.titleWithName', { name })
              : t('boldFlow.diagnosis.titleNoName')}
            {' '}
            <span style={{ color: level.color }}>{score}%</span>
            {' '}
            {t('boldFlow.diagnosis.titleScoreSuffix')}{' '}
            <span style={{ color: level.color, background: level.bg, padding: '0 6px' }}>
              {t('boldFlow.diagnosis.titleHighlight')}
            </span>
          </h1>
        </FadeIn>
        ...
        <FadeIn delay={0.4}>
          <div className="flex flex-col gap-3">
            <p className="text-xs font-bold uppercase tracking-wider text-stone-500 text-center">
              {t('boldFlow.diagnosis.factorsHeading')}
            </p>
        ...
        <FadeIn delay={0.6}>
          ...
          >
            <p className="font-extrabold text-base leading-snug" style={{ color: level.color }}>
              ⚠ {t('boldFlow.diagnosis.urgentBanner')}
            </p>
          </div>
        </FadeIn>
        ...
        <FadeIn delay={0.7}>
          <div className="bg-white rounded-2xl p-5 border border-stone-200 flex flex-col gap-3">
            <p className="text-base font-extrabold text-stone-900 text-center">
              {t('boldFlow.diagnosis.previewHeading')}
            </p>
            <ul className="flex flex-col gap-2.5">
              {t('boldFlow.diagnosis.previewItems', { returnObjects: true }).map((item, i) => (
        ...
        <FadeIn delay={0.9}>
          <p className="text-xs text-stone-400 text-center">
            {t('boldFlow.diagnosis.reassurance')}
          </p>
        </FadeIn>
      </div>
