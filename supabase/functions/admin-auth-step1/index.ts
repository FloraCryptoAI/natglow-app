    ...
    body: JSON.stringify({
      ...
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:400px;margin:0 auto;padding:32px">
          ...
          <p style="color:#d6d3d1;font-size:12px;margin-top:16px">Se você não solicitou este código, ignore este email.</p>
        </div>`,
    }),
  })
  return res.ok
...
Deno.serve(async (req) => {
  ...
  try {
    if (!ADMIN_PASSWORD_HASH) return json({ error: 'Sistema não configurado. Configure ADMIN_PASSWORD_HASH.' }, 503)
    ...
    const emailOk = emailAttempted === ADMIN_EMAIL.toLowerCase().trim()
    ...
    if (sessErr || !session) {
      await audit(ip, emailAttempted, 'step1', 'session_error')
    ...
    if (!sent) {
      ...
      return json({ error: 'Erro ao enviar código. Verifique RESEND_API_KEY e tente novamente.' }, 500)
    }
